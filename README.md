...
- 光敏性癫痫患者、易受惊吓者请勿游玩。
- 经你确认内容警告后，部分场景会在桌面上创建临时本地故障窗口（最多 4 个，最长 12 秒，退出剧本即统一关闭），并在 `data` 同级的 `characters/standalone/第七个测试剧本/` 写入两个无害 `.chr` 剧情标记；不会执行外部命令、模拟真实系统崩溃或修改 LingChat 角色模型。
- 游玩过程中如感到不适，请立即退出。
...
本 DLC 使用了剧本引擎的扩展事件（`jumpscare` / `force_choice` / `poem_game` / `voice_shift` / `character_file` / `main_menu_effect` / `glitch_window` 等）与 DLC 识别功能。后三项均由引擎做固定预设、剧本归属和路径白名单校验，旧引擎无法完整运行 v2.1：

- 上游支持 PR：[SlimeBoyOwO/LingChat#677](https://github.com/SlimeBoyOwO/LingChat/pull/677)（合并发版后，标注此处需要的最低版本）
- 最低版本：LingChat `0.5.1`；新引擎会在导入/扫描时强制校验 `dlc.json.min_engine`，旧版 `0.5.0` 不得用于 v2.1
