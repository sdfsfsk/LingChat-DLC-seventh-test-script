...
**v1.1.1 更新**：崩坏段语音全面低沉化——所有 `voice_shift` 叠加纯音调偏移（pitch -3 ~ -5 半音，语速与音调分离，配合播放倍率实现真正的"恶魔音"）；摊牌/重启段心跳底噪降速 0.9x。⚠️ 本版本的 pitch 参数需要包含 [#677](https://github.com/SlimeBoyOwO/LingChat/pull/677) 最新构建的引擎（旧引擎会忽略 pitch、仅保留原降速效果，向后兼容）。
