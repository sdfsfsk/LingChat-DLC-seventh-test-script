...
| ghost menu（删完全部 .chr） | 引擎级"幽灵锁定"：进入剧本被锁成黑白恐怖立绘（`Assets/Pics/ghost-ql-bw.webp`，codex 生成的钦灵 menu_art_m_ghost 同款空心黑眼黑白风）+ ghostmenu.ogg 循环，全 UI 只剩「重置记忆」；放回任一 .chr 2 秒内自动解锁，重置=记忆+角色文件全恢复；锁定中点窗口 X → 白底放大脸突脸（DDLC `label quit` 的 zoom 3.5 同款）+ s_kill_glitch1 后真正退出 ✅ |
